...
module LayersInTime
  ...
  module HTMLDialogs
    ...
    # Utility that converts a SketchUp color to a HTML RGB color code.
    #
    # @param [Sketchup::Color] color
    # @raise [ArgumentError]
    #
    # @return [String] HTML RGB color code.
    def self.html_rgb_color(color)

      raise ArgumentError, 'Color must be a Sketchup::Color.'\
        unless color.is_a?(Sketchup::Color)

      'rgb(' + color.red.to_s + ', ' + color.green.to_s + ', ' + color.blue.to_s + ')'
